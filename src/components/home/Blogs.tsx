...
export default function Blogs() {
  return (
    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-8 space-y-12">
      {BlogList.map((item) => (
        <li key={item.id} className="h-[693px]">
          <Image
            src={item.image}
            width={505}
            height={475}
          ...
          <button className="flex items-center py-2 gap-4 mt-4">
            <p className="text-[#667F93] text-lg font-semibold">
              Khám phá thêm
            </p>
            <Image
